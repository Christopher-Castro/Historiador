<template>
  <div :class="baseClassName" @mousewheel="onMouseWheel">
    ...
    <div class="label" v-if="label">
      <div class="label-min">{{ barMinText }}</div>
      <div class="label-max">{{ barMaxText }}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    barMinText() {
      return moment(this.valueMin).format("YYYY-MM-DD HH:mm:ss");
    },
    barMaxText() {
      return moment(this.valueMax).format("YYYY-MM-DD HH:mm:ss");
    }
  },
  watch: {
    valueMin() {
